...
class Game: 
    ...
    def findWinner(self):
        ...
        if self.state.board['M1'] > self.state.board['M2'] :
            ...
            return 3, self.state.board['M2']   #player 2 won
...
class Play:
    ...
    def humanTurn(state :Game, curent_player, move):
        ...
        return curent_player, state 

    def computerTurn(game : Game, curent_player, depth = 6 ):
        if len(game.state.possibleMove(curent_player)) > 0:
            best_node = MinMaxAlphaBetaPruning(game, game.playerSide[curent_player], depth, game.alpha, game.beta)
            curent_player = game.state.doMove(curent_player, best_node[1])

        return curent_player, game , best_node[1]
